(ns pizzabom.routes.fk-api
  (:require
   [compojure.core :refer [defroutes GET POST]]
   [pizzabom.engine.config :as config]
   [pizzabom.engine.crud :as crud]
   [pizzabom.models.crud :as model-crud]
   [pizzabom.models.util :refer [json-response]]
      ...
      (try
        (let [entity-config (config/get-entity-config entity)
              ...
              form-fields (map #(select-keys % [:id :label :type :required? :placeholder
                               ...
                               fields)
              rendered (let [render-fn #'pizzabom.engine.render/render-field]
